...
public class TransactionsDaoImpl implements TransactionsDao{
	...
	public LocalServerPOJO getLocalServerByToken(Connection con, String token) throws SQLException
	{
		...
		if(newCon == null)
			newCon= DBConnection.getConnection();
		String query= "select * from ServerDetails"
				+ " where token= ?";
		LogUtil.logger.info("Query: "+query);
		try {
			ps= newCon.prepareStatement(query);
	...
	@Override
	public LocalVehiclePassPOJO getVehiclePass(Connection con, String fullLicencePlate) throws SQLException
	{
		Connection newCon= con;
		PreparedStatement ps= null;
		ResultSet rs= null;
		if(newCon == null)
			newCon= DBConnection.getConnection();
		String query= "select * from LocalVehiclePass where full_licence_plate= ?";
		LogUtil.logger.info("Query: "+query);
		try {
			ps= newCon.prepareStatement(query);
			int i= 1;
			...
			while(rs.next())
			{
				LocalVehiclePassPOJO vehpass= new LocalVehiclePassPOJO();
