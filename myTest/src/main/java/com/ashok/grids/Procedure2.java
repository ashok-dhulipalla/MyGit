...
/*		gridName="LTVRestrictionGrid"; 
	...
	private static String getColumnDetail(String gridName,int select) {
		...
		columnDetail+=unique+++"-"+select+"-"+spacesForGridNames(ShortCutReplace(gridName))+"-bucket_code-0-NULL-NULL-Output\n";
		return columnDetail;
	}
	...
	private static String getView(String gridName,String gridColumns) {
		...
		view+=", bucket_code, bucket_name, gridversion_eid from sangam_mdm."+gridName+";\n"
				+ "GO\n";
		return view;
	}
	private static String getTemplate(String gridName, String gridColumns, String masterNames) {
		...
		template+=", bucket_code, valid_version_id) VALUES ( {1}";
		int j=2;
		for(int i=0;i < gridcolumnslist.length;)
		{
			template+=",{"+j+"}";
		...
		template+=", bucket_code AS \""+j+++"\", bucket_name AS \""+j+++"\", unique_id AS \""+j+++"\" FROM "+gridName+" where version_id = '{version_id}' ]]></select-query>\n";
		template+="    <properties>\n" + 
				"		<property sequence=\"1\">\n" + 
		...
		template+="		<property sequence=\""+j+++"\">\n" + 
				"			<property-type></property-type>\n" + 
				"			<mandatory>true</mandatory>\n" + 
				"			<property-name>bucket_code</property-name>\n" + 
				...
				"			<property-type>ignore</property-type>\n" + 
				"			<mandatory>false</mandatory>\n" + 
				"			<property-name>bucket_name</property-name>\n" + 
	...
	private static String getCopyProcedure(String gridName, String gridColumns) throws IOException {
		...
		copy+=",BUCKET_CODE, BUCKET_NAME, ENTITYID, ENTITYVERSION, ACTIVE, LASTMODIFIEDTIME, BUSINESS_STATUS, CREATED_BY, CREATION_DATE, LAST_MODIFIED_BY, LAST_MODIFIED_DATE, OWNER, OWNER_NAME, READ_ONLY, URL, IR_LAST_APPROVED_BY, IR_APPROVAL_STATUS, IR_LAST_APPROVED_DATE, LCM_STATUS) \n" + 
				"		SELECT UNIQUE_ID, MASTER_ID, MASTER_NAME, VERSION_ID, GRIDVERSION_EID, PRODUCT_CODE, PRODUCT_NAME";

		copy+= getAllColumns(gridcolumnslist);

		copy+=", BUCKET_CODE, BUCKET_NAME, UNIQUE_ID, '0', 'A', 'NULL', 'Active', 'System', GETDATE(), 'System', GETDATE(), 'a', 'Common', '1', 'NULL', 'NULL', 'approved', null, null FROM "+gridName+"Staging; \n" + 
				"\n" + 
	...
	private static String getStagingTable(String gridName, String gridColumns) {
		...
		tables+="\n	\"bucket_code\" NUMERIC (18),\n" + 
				"	\"bucket_name\" VARCHAR (4000),\n" + 
				"	\"status\" VARCHAR (100),\n" + 
	...
	private static String getProcedure(String gridName, String gridColumns, String masterColumns,String masterNames)
	{
		...
		for(int i= 0;i < gridcolumnslist.length;)
		{
			...
			if(!masternameslist[i].equals("CombiBucket"))
				...
				procedure+="WHERE arcgs."+gridcolumnslist[i]+" IS NOT NULL AND NOT EXISTS ( select bucket_code from CombiBucket where  "+mastercolumnslist[i]+" = arcgs."+gridcolumnslist[i]+" /*and is_active = 1*/ );\n";
			i+= 2;
		...
		procedure+="UPDATE "+gridName+"Staging  \n" + 
				...
				"WHERE bucket_code IS NULL;		\n" + 
				"\n" + 
				
				"UPDATE arcgs \n" + 
				"SET arcgs.status = 'ERROR', \n" + 
				"	arcgs.error_message = ISNULL(arcgs.error_message,'') + 'Combination Bucket Code should be present in CombiBucket Master,' \n" + 
				"FROM "+gridName+"Staging arcgs\n" + 
				"WHERE arcgs.bucket_code IS NOT NULL AND NOT EXISTS ( select bucket_code from CombiBucket where  bucket_code = arcgs.bucket_code /*and is_active = 1*/ );\n" + 
				"\n" + 
		...
		procedure+="		crcgs.bucket_name = buck"+j+".bucket_name \n";
		...
		procedure+="	INNER JOIN CombiBucket buck"+j+" on crcgs.bucket_code = buck"+j+".bucket_code\n" + 
		...
		procedure+=",BUCKET_CODE, BUCKET_NAME, ENTITYID, ENTITYVERSION, ACTIVE, LASTMODIFIEDTIME, BUSINESS_STATUS, CREATED_BY, CREATION_DATE, LAST_MODIFIED_BY, LAST_MODIFIED_DATE, OWNER, OWNER_NAME, READ_ONLY, URL, IR_LAST_APPROVED_BY, IR_APPROVAL_STATUS, IR_LAST_APPROVED_DATE, LCM_STATUS) \n";

		procedure+="	SELECT UNIQUE_ID, MASTER_ID, MASTER_NAME, VERSION_ID, GRIDVERSION_EID, PRODUCT_CODE, PRODUCT_NAME";

		procedure+= getAllColumns(gridcolumnslist);

		procedure+=",BUCKET_CODE, BUCKET_NAME, UNIQUE_ID, '0', 'A', 'NULL', 'Active', 'System', GETDATE(), 'System', GETDATE(), 'a', 'Common', '1', 'NULL', 'NULL', 'approved', null, null FROM "+gridName+"Staging \n" + 
